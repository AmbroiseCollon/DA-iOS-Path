...
Choisissez ensuite l'option *Swift File* et cliquer sur *Next* :
...
A vous de jouer maintenant créer la classe `Question` avec pour propriété `title` de type `String` et `isCorrect` de type `Bool`.
...
- **Type par référence** : lorsqu'on initialise une instance de classe, on déclare à la mémoire de notre ordinateur : "*Trouve moi un endroit où je vais pouvoir stocker un objet*". L'ordinateur s'execute et renvoie une adresse en mémoire où l'instance va pouvoir stocker des informations. Donc quand on écrit : `var instance = MaClasse()`, la variable `instance` ne contient pas un objet mais seulement une adresse en mémoire (une *référence*) à laquelle se trouve l'objet.
...
Cette fois, ça ne marche pas ! **Une instance d'une structure déclarée avec let a ses propriétés constantes**. Tout est constant et plus rien ne peut être modifié. Car les structures représentent directement leur valeurs, donc si la structure est constante, toutes les valeurs contenues le sont aussi.
...
Hormis ces deux grandes différences, il existe quelques différence pratiques et je vais vous en citer deux principales :

##### 1. L'initialisation par défaut
Si on reprends notre structure `RectangleStruct`, on peut lui enlever ses valeurs par défaut et son initialisation et contrairement à la classe, cela ne déclenchera pas d'erreur :
...
La raison, c'est que les structures ont une petite fonctionnalité bien pratique. Elles sont un équipées automatiquement d'un **initialiseur par défaut qui prends en paramètre ses propriétés stockées**. Autrement dit, sans avoir déclaré un initialiseur, je peux écrire :
```swift
...
> **:information_source:** Ne vous faîtes pas de souci, si vous l'oubliez, une erreur vous le rappellera sans tarder !
...
- Une structure ressemble beaucoup à une classe avec 2 différences fondamentales :
	- les structures n'ont **pas d'héritage**.
	- les structures sont un **type par valeur** et les classes un **type par référence**.
- Utiliser une structure lorsque vous chercher principalement à stocker des données.
...
Et on commence très fort ce chapitre avec déjà un exercice ! En effet, vous savez très bien créer des classes, donc je vous propose de la créer sans moi dans cet exercice. Faîtes le vraiment sinon vous risquez d'être perdus lorsqu'on devra utiliser notre classe `Game`.

[Exercice Codevolve]

Un fois l'exercice terminée, vous pouvez télécharger le fichier [Game.swift](https://s3-eu-west-1.amazonaws.com/static.oc-static.com/prod/courses/files/Parcours+DA+iOS/Cours+4+-+MVC/Game.swift) et le glisser dans votre modèle (c'est la même opération que ce qu'on a fait avec la police Balham).

#### QuestionManager
Notre quizz va charger ses questions depuis internet. Plus précisément, nous allons charger nos questions depuis la base de donnée de questions gratuite : [Open Triva Database](https://opentdb.com). Les requêtes réseaux ne sont pas au programme de ce chapitre donc je l'ai fait pour vous. Vous pouvez donc télécharger le fichier : [QuestionManager.swift](https://s3-eu-west-1.amazonaws.com/static.oc-static.com/prod/courses/files/Parcours+DA+iOS/Cours+4+-+MVC/QuestionManager.swift). Vous pouvez ensuite glisser ce fichier dans votre modèle.

Ce fichier est un peu compliqué et nous n'allons pas rentrer dans le détail de ce qu'il fait. Sachez seulement qu'il récupère 10 questions dans la base de donnée, les formatte et les renvoie. Il le fait via la seule méthode publique  de cette classe qui a pour signature `get(completionHandler: @escaping ([Question]) -> ())`. Et c'est cette méthode que nous allons utiliser pour finaliser la méthode `refresh` de notre classe `Game`.
...
Bon OK, ça peut paraître un peu impressionant mais rassurez-vous. À la fin de chapitre, cela n'aura plus de secret pour vous.
...
```
Ce n'est donc pas plus compliqué que ça ! Donc si on fait le travail inverse : le type `([Question]) -> ()` décrit une fonction qui prends en paramètre un tableau de questions et qui ne renvoie pas de valeur.
...
Ces quatres fonctions sont du même type : `(Int) -> Int`. Donc on peut créer un tableau qui les regroupe toutes :
```swift
...
Exactement ! Vous êtes impressionant ;) !
...
Excellente question ! En fait, cette fonction est un peu particulière car on ne sait pas quand la valeur de retour va arriver. Cela va dépendre de la qualité du réseau et du temps de réponse du serveur etc. Autrement dit, dès qu'on va chercher des ressources sur internet, on est obligé d'**attendre la réponse**. Hors une fonction avec valeur de retour s'exécute instantanément et donc les questions n'ont pas le temps d'être chargées.
...
Une fermeture **commence par une accolade**. Ensuite on lui passe les paramètres et les valeurs de retours comme une fonction normale. Ensuite on ajoute le mot clé `in`. Et ensuite vient le corps de la fermeture terminé par une accolade.
...
Ensuite, les fonctions qui terminent par une fermeture admette une syntaxe simplifiée. On peut omettre le paramètre `completionHandler` et écrire la fermeture en dehors des parenthèses :
...
> **:warning:** Vous noterez que j'utilise le mot-clé `self` systématiquement. Dans une fermeture, on perds le contexte dans lequel on était. On doit donc toujours le lui rappeler en passant le mot-clé `self` à chaque fois que l'on veut utiliser une méthode ou une propriété de la classe dans laquelle on se trouve.
...
Commençons par créer le nom de notre notification. Ce nom c'est un peu comme la fréquence de ma radio. **Tout ceux qui veulent écouter ma radio devront utiliser le même nom** :
```swift
...
On utilise ensuite la méthode `post` qui prends en paramètre la notification que nous venons de créer. A la fin, notre méthode refresh ressemble à ceci :
...
> **:information_source:** Il existe une autre version de la méthode `post` qui prends en paramètre le nom directement. Cela nous permet d'écrire tout cela en une (grande) ligne. C'est pratique mais un peu moins lisible :
```swift
